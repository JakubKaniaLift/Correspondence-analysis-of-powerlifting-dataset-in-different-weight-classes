...
###Heat-map
heatmap(female_best_m,scale="none",Colv=NA,col= brewer.pal(8,"Oranges"))
...
#_______________________________________________________________________________
#Heatmaps for best 20: men 93kg class and female 63Kg class
#Men
men_100 <- lifts_m %>% 
...
#Female
female_100 <- lifts_f %>%
  filter(Weight_class == "52") %>%
  arrange(desc(Wilks))

female_100

table(lifts_m[lifts_m$Date == '2019-06-04',]$Weight_class)
table(lifts_f[lifts_f$Date == '2019-06-04',]$Weight_class)


#Men - '2019-06-04'
men_100 <- lifts_m %>% 
  filter(Date == '2019-06-04',Weight_class == '93') %>%
  arrange(desc(Wilks))
...
#Female - '2019-06-04'
female_100 <- lifts_f %>% 
  filter(Date == '2019-06-04',Weight_class == '63') %>%
  arrange(desc(Wilks))

female_100

men_100_m <- as.matrix(men_100[1:20,as.numeric(c(4,5,6))])
female_100_m <- as.matrix(female_100[1:20,as.numeric(c(4,5,6))])
